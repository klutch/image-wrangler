...
## The tree lives in scenes/iw_color_list.tscn; this fetches it and wires it up.
##
...
func _build() -> void:
    ...
    _list = %List
    _editor = %Editor
    _color_button = %ColorButton
    _mixed_label = %MixedLabel
    _picks_toggle = %PicksToggle
    _samples_box = %SamplesBox
    _hint = %Hint
    ...
    _editor.move_child(_tolerance_slider, 1)
